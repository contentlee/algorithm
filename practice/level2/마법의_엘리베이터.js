...
// 예를 들어, 16층에 있는 민수가 0층으로 가려면 -1이 적힌 버튼을
// 6번, -10이 적힌 버튼을 1번 눌러 마법의 돌 7개를 소모하여 0층으로 갈 수 있습니다.
...
function solution(storey) {
  const queue = [];
  queue.push({ val: storey, cnt: 0 });

  let idx = 0;

  while (idx < queue.length) {
    const { val, cnt } = queue[idx];

    idx++;
    if (val < 0) continue;
    if (val === 0) break;

    const remainer = val % 10;
    const share = Math.floor(val / 10);

    if (remainer === 5) {
      if (share % 10 >= 5) {
        queue.push({ val: share + 1, cnt: cnt + 10 - remainer });
      } else {
        queue.push({ val: share, cnt: cnt + remainer });
      }
    } else if (remainer < 5) {
      queue.push({ val: share, cnt: cnt + remainer });
    } else {
