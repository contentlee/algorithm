...
function solution(weights) {
  ...
  weights.forEach((weight) => {
    if (!table[weight]) table[weight] = 0;
    table[weight]++;
  });
  ...
  for (const w in table) {
    if (table[w] > 1) answer += sumN(table[w] - 1);
    if (table[w * 2]) answer += table[w * 2] * table[w];
    if (table[w * (3 / 2)]) answer += table[w * (3 / 2)] * table[w];
    if (table[w * (4 / 3)]) answer += table[w * (4 / 3)] * table[w];
  }
