...
function solution(lines) {
  const logs = lines.map((line) => {
    const [_, time, prc] = line.split(" ");
    const [h, m, s] = time.split(":").map((v) => +v);
    const [prcT] = prc.split("s");

    const end = h * 60 * 60 + m * 60 + s;
    const start = (end * 10 ** 3 - Number(prcT) * 10 ** 3 + 1) / 1000;
    return [start, end];
  });
  let answer = 0;
  for (let i = 0; i < logs.length; i++) {
    let tmp = 1;
    let nxt = (logs[i][1] * 10 ** 3 + 999) / 10 ** 3;
...
// 처음에는 말그대로 초의 정수범위까지만 생각했었는데, 소수점까지 포함되는 문제여서 약간의 고전이 있었다.
// 이미 lines는 오름차순으로 정렬되어 있기 떄문에, 반복문을 돌리며 1초 안에 다음 요청이 포함되는지만 확인하면 되었다.
